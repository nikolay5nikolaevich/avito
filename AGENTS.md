...
- VPN-конфликт: Авито нужен РУ-IP, Codex — не-РУ. Разводим по времени
...
## План
**Готово (MVP):** окружение + парсер одного/всех городов + метрики + кэш/CSV/прогресс,
произвольное количество объявлений (1–500), фильтр цены (`pmin`/`pmax`), выбор городов
(топ-50, дефолт топ-10), фильтр пола (слово в запросе), фильтр локальности (отсев чужих
регионов). React-фронт (`frontend/`) + JSON API (`/api/*`). Бэкенд вынесен в `backend/`.
- [ ] (после MVP) масштаб до 170 городов, прокси

**Сейчас:** MVP готов, самотесты модулей и smoke зелёные. Параметры цены `pmin`/`pmax`
ПОДТВЕРЖДЕНЫ на живом Авито (2026-05-26): легаси-параметры работают, хотя новый UI Авито
генерирует сложный `f`-блоб. Серверный Jinja2-фронт удалён, интерфейс — React.
ВАЖНО: `python backend/app.py` запускает uvicorn БЕЗ авто-перезагрузки — после правок кода
сервер надо перезапускать вручную. ВАЖНО-2: при изменении логики метрик (analytics.py) ключ
кэша не меняется → удаляй `cache.db`, иначе отдаст старый результат. Остаётся ручная проверка
...
## Claude Feedback
После каждого выполненного брифа/задачи обязательно создавать отдельный файл в `docs/claude-feedback/`.
Имя файла: `YYYY-MM-DD-<slug-задачи>.md` — по дате и смыслу брифа, чтобы можно было быстро найти нужную сессию.
Цель файла: дать следующему агенту или Claude быстрый вход без повторного чтения всего проекта.
В каждом таком файле обязательно кратко описывать:
- что именно было сделано;
- какие файлы изменялись;
- как теперь работает результат;
- что и как проверялось;
- какие есть ограничения, риски или что важно не забыть в следующей сессии.
Писать коротко, по делу, так чтобы по одному этому файлу можно было понять суть выполненной работы.
